...
public class Path implements Iterable<Location> {
     ...
     * @param y The y coordinate of the loation
     * @return True if the given loation is in the path, and false otherwise
     */
    ...
    /**
     * Returns the symbol the represents the given locaiton on the path.
     * @param x The x coordinate of the location
     ...
     * and the next pint from the current p[oint.
     ...
     * @return They symbol that represents the given location on the path
     */
    private char getSymbolFromDirections(Direction from, Direction to) {
        ...
        if(from == Direction.NORTH && to == Direction.EAST || from == Direction.WEST && to == Direction.SOUTH) {
            return '╔';
        }
        if(from == Direction.NORTH && to == Direction.WEST || from == Direction.EAST && to == Direction.SOUTH) {
            return '╗';
        }
        return ' ';
    }
